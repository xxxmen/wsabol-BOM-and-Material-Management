VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} ChooseCheckCopySites 
   Caption         =   "PDF Check Copy Sites"
   ClientHeight    =   2040
   ClientLeft      =   45
   ClientTop       =   375
   ClientWidth     =   2910
   OleObjectBlob   =   "ChooseCheckCopySites.frx":0000
End
Attribute VB_Name = "ChooseCheckCopySites"
...
''''''''''''''''''''''''''''''''''''''''''''
' User Form: ChooseCheckCopySites          '
'                                          '
    ...
    For Each chbx In Me.Controls
        If TypeName(chbx) = "CheckBox" And chbx.Caption <> "All Site(s)" Then
            If Not chbx.Value Then
                ...
                If SiteExists(chbx.Caption, mst_site_col) Then
                    ' not crazy
                End If
                
                dftg_site_col = VB_DFTGCHECK.QuantitiesColumn() + mst_site_col - get_col_num("Current Model Quantities")
                
                VB_DFTGCHECK.Columns(dftg_site_col).Hidden = True ' hidden; won't print
                VB_DFTGCHECK.Columns(dftg_site_col + rSites.count).Hidden = True ' hidden; won't print
            End If
        End If
    Next chbx
